...
void setup() {
  ...
  tft.setCursor(50, 40, 4);  // Set "cursor" at top left corner of display (30,40) and select font 4
  tft.println("Tanks - Allies vs Axis");  // Print Encoder Count
  tft.setCursor(0, 80, 4);  // Set "cursor" at top left corner of display (30,40) and select font 4
  tft.println("  Move with Joystick");  // Print Encoder Count
  tft.println("  Aim with Encoder");  // Print Encoder Count
  tft.println("  Fire with BlueButton");  // Print Encoder Count
  tft.println("  Press BlueButton");  // Print Encoder Count
  tft.println("     to Continue");  // Print Encoder Count
  while (digitalRead(bPB) == HIGH) {}
  tft.fillScreen(color);
...
void shellControl() {
  // User Tank Fire
  if (digitalRead(bPB) == 0 && !shellFired)  //Initiate a Firing with the blue button
...
void checkForDamage() {
  ...
  if (abs((eTankX + 25) - shellX) < killRad && abs((eTankY + 25) - shellY) < killRad)
  {
    ...
    {
      tft.fillCircle(eTankX + 25, eTankY + 25, x, TFT_BLUE);
      delay(20);
    }
  ...
  if (abs((tankX + 25) - eShellX) < killRad && abs((tankY + 25) - eShellY) < eKillRad)
  {
    ...
    {
      tft.fillCircle(tankX + 25, tankY + 25, x, TFT_BLUE);
      delay(20);
    }
    //    delay(1500);
