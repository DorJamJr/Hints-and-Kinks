/*
   simple sketch for writing sample strings to the SD card, reading them back,
   and deleting them. At the beginning of the sketch are 3 pangrams – an
   interesting sentence that uses every one of the 26 letters of the alphabet at least once.
   Pangrams are often used by programmers to test displays and printers.

   The program getone of 3 commands (r,w and d) from the Serial Monitor
   to execute the three functions.
   https://www.circuitbasics.com/writing-data-to-files-on-an-sd-card-on-arduino/
*/

//*************   NOTE:  This program does not work as expected.  It needs WORK!
